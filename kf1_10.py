...
class kalmanFilter:
	...
	def calcDistance(self, d, o, a, v, t):
		...
		#update distance
		d[0] = d[0] + v[0]*t + (a[0]*t*t)/2
		d[1] = d[1] + v[1]*t + (a[1]*t*t)/2
		d[2] = v[2] + v[2]*t + (a[2]*t*t)/2
...
def updateSensors():
	...
	'''
	accel = acc.get_acc_all()
	gyro = acc.get_gyo_all()
	#accel = [0,0,-1]
	#gyro = [0,0,0]
	return accel, gyro
...
def main():
	...
	while i<1000:
		...
		acceleration = [round(acceleration[0],3),round(acceleration[1],3),round(acceleration[2],3)]
		...
		print ("Rounded A", "%0.3f" % acceleration[0],"%0.3f" % acceleration[1],"%0.3f" % acceleration[2]) #just for our benefit
		print ("Rounded V", "%0.3f" % velocity[0],"%0.3f" % velocity[1],"%0.3f" % velocity[2])
		print ("Rounded D", "%0.3f" % distance[0],"%0.3f" % distance[1],"%0.3f" % distance[2])
		print ("---")
